...
# Function to fix special characters in JSON string
def fix_special_characters(json_str):
    for key, value in json_str.items():
        if isinstance(value, str):
            json_str[key] = value.replace("\\u00e8", "è")  # Replace special characters with their encoded equivalents
...
def process_json_response(json_str):
    try:
        # Ensure JSON is in string format
...
def _generate_new_translations(user: entity.User, tutor: entity.Tutor):
    ...
    if start:
        ...
        data = process_json_response(data)
